...
class DataCleaning:
    ...
    def replace_other_columns(
        ...
        for column, other_column in mapping.items():
            ...
            if (self.df[column].dropna().apply(isinstance, args=((list, np.ndarray),))).any():
                self.df[column] = [self.th.replace_array_item(index, value, self.df, other_column, other_value)
    ...
    def remap_categorical_labels(self, columns: list[str] = None) -> None:
        """
        ...
        for col in columns:
            if col in self.df.columns:
                ...
                self.df[col] = self.df[col].apply(partial(self.th.replace_label, mapping=mapping))
    ...
    def enforce_dtype(self, dtype_map: dict[str, list[str]]) -> None:
        """
    ...
    def rename_features(self, mapping: dict[str, str]) -> None:
        """
    ...
    def drop_features(self, columns: str | list[str]) -> None:
        """
    ...
    def _create_other_columns_mapping(self, search_str: str) -> None:
        ...
        if not mapping:
            raise ValueError(f'No other columns found in the codebook to replace with {search_str}.')

        self.other_columns_map = mapping

    def _check_numeric(self, column) -> tuple[pd.Series, list]:
    ...
    def scale_birth_weight(self, column: str = 'peso_nascimento') -> None:
        """
        Scale the birth weight values to fix typing errors.

        Parameters
        ----------

        column : str, optional
            The name of the column containing the birth weight values. Defaults to 'peso_nascimento'.

        """
        self._scale_values(column, 4, 10)
        self._scale_values(column, 3, 100)
        self._scale_values(column, 2, 1000)
        self._scale_values(column, 1, 1000, period=False)
    ...
    def recalculate_age(
        ...
        """
        Recalculates the age based on the specified columns.

        Parameters
        ----------
        age_column : str
            The name of the column containing the age values.
        age_unit_column : str
            The name of the column containing the age unit values.
        new_column : str
            The name of the new column to store the recalculated values.
        age_unit_map : dict, optional
            The dictionary mapping age units to the number of days in the unit.
            If not provided, the method will use the default mapping.
    ...
    def split_code_descrpition(self, *args):
        """Helper method to apply the split_code_descrpition method to multiple columns."""
        for kwargs in args:
    ...
    def apply_split_code_descrpition(
        ...
        """
            Applies the split_code_description method to a specified column.

            This method extracts the code and description from the specified column based on the
            provided filter pattern. It then inserts these extracted values into new columns in the DataFrame.
            The new columns are inserted immediately after the specified column.
            The original column is then dropped from the DataFrame.

            Parameters
            ----------
            column : str
                The name of the column to apply the split_code_description method to.
            filter_pattern : str
                The pattern to use for filtering the column values.
            code_name : str
                The name of the new column to store the extracted code values.
            desc_name : str
                The name of the new column to store the extracted description values.

            Raises
            ------
            ValueError
                If the code column cannot be cast to an integer.
            """
        # Raise error if column is not object type
        if not pd.api.types.is_object_dtype(self.df[column]):
            raise ValueError(f'Column {column}({self.df[column].dtype}) is not of object type.')

        # Extract code and description
        self.df[column] = self.df[column].dropna().apply(
            lambda x: self.th.extract_ids_and_descriptions(x, filter_pattern))
    ...
    def check_cpf(self):
        """Check for invalid CPF values in the DataFrame."""
        invalid_cpf_mask = self.df['cpf'].dropna().apply(self.th.validate_cpf).isna()
    ...
    def check_cns(self):
        """Check for invalid CNS values in the DataFrame."""
        invalid_cns_mask = self.df['cns'].dropna().apply(self.th.validate_cns).isna()
